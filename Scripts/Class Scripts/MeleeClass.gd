...
@export var weaponStrikeObject : Node2D ## melee collider box that detects hits with enemies 
...
func _ready():
	...
	weaponStrikeObject.disabled = true
	AnimationEvents(0)
...
func Swing(delta): 
	...
	angularMomentum = clampf(abs(angularMomentum), -maximumMomentum, maximumMomentum)
	swingAngle += angularMomentum
	global_rotation = swingAngle
	print(angularMomentum / delta / 3.0)

func StartSwing(delta):
	swinging = true 
	weaponStrikeObject.disabled = false  # enabling collision for the Melee weapon collider (disabled by default as its holstered) 
...
func StopSwing():
	...
	angularMomentum = 0.0
	swinging = false
	weaponStrikeObject.disabled = false
